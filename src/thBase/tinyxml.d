...
protected{
	...
	/*static long GetEntity( char[] sin, ref char[] value, ref int length, TiXmlEncoding encoding )
	{
	...
	static long GetChar( char[] sin, ref char[] _value, ref int length, TiXmlEncoding encoding )
	in { assert(sin); }
	body
	{
		if ( encoding == TiXmlEncoding.TIXML_ENCODING_UTF8 )
  ...
  IAllocator m_allocator;
	...
	static void ConvertUTF32ToUTF8( dchar input, char[] output, ref int length )
	{
		...
		switch (length) 
		{
			case 4:
				--o; 
				output[o] = cast(char)((input | BYTE_MARK) & BYTE_MASK); 
				input >>= 6;
        ...
        goto case; //fallthrough
			case 1:
				--o; 
				output[o] = cast(char)(input | FIRST_BYTE_MARK[$]);
