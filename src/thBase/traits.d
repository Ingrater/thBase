...
// gets a field ignoring all protection attributes
template getField(T, string name)
{
  alias getField!(T, name, 0) getField;
}

template getFieldImpl(T, string name, size_t i)
{
  ...
  static if(i >= T.tupleof.length)
  {
    ...
    {
      alias getFieldImpl = getFieldImpl!(BaseClass!T, name, 0);
    }
    else
      static assert(is(T == Object), "member " ~ name ~ " could not be found");
  }
  else
  {
    ...
    {
      alias getFieldImpl = T.tupleof[i];
    }
    else
      alias getFieldImpl = getFieldImpl!(T, name, i + 1);
  }
}